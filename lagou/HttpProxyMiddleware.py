...
class HttpProxyMiddleware(object):
    ...
    def __init__(self, settings):
        ...
        # self.proxyes = [{"proxy": None, "valid": False, "count": 0}]
        # 初始时使用0号代理(即无代理)
        self.proxy_index = 0
    ...
    def inc_proxy_index(self):
        ...
        logger.info("now using new proxy: %s" %
    ...
    def process_request(self, request, spider):
        ...
        if self.proxy_index > 0 and datetime.now() > (self.last_no_proxy_time + timedelta(minutes=self.recover_interval)):
            logger.info("After %d minutes later, recover from using proxy" %
